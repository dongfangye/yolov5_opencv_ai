...
for filename in os.listdir(folder_path):
    if filename.endswith('.jpg') or filename.endswith('.png'):
        ...
        folder_images.append((filename, extract_features(image)))
    ...
    for i in range(n):
        if int(labels[i]) == target_label_index:  # 只处理目标标签
            ...
            if row[4] >= 0.2:  # 设置置信度阈值
                ...
                # 与文件夹中的图像进行对比
                for filename, folder_features in folder_images:
                    similarity = compare_images(cropped_features, folder_features)
